...
int EvHttpClient::init() {
    ...
    if (err < 0) {
        ...
        ss_error << "Error: couldn't resolve host name. host:" << url.host() << ", port:" << url.port() << endl;
        last_error = ss_error.str();
        free(host);
        free(service);
        return -1;
    }
...
const char *EvHttpClient::lastError() {
...
HttpConn *EvHttpClient::createConn() {
    ...
    if ((conn->fd = socket(family, socktype,
            protocol)) < 0) {
        last_error = "socket() failed: " + string(strerror(errno));
        delete conn;
        return NULL;
    }
    ...
    if ((connect(conn->fd, &addr, addrlen)) < 0) {
        if (errno != EINPROGRESS) {
            last_error = "connect() failed: " + string(strerror(errno));
            delete conn;
            return NULL;
        }
    }
...
void EvHttpClient::returnConn(HttpConn *conn) {
    ...
    if (connections.size() < init_num_conns) {
...
void EvHttpClient::writeCb(struct ev_loop *loop, struct ev_io *watcher, int revents) {
    if (EV_ERROR & revents) {
        last_error = "got invalid write event: " + string(strerror(errno));
        return;
    }
...
//    char *toWriteC = strdup(toWrite.c_str());
    char *toWriteC = (char*)malloc(sizeof(char) * toWrite.size());
    memcpy(toWriteC, toWrite.c_str(), toWrite.size());

    int sent = send(watcher->fd, toWriteC, toWrite.size(), 0); //MSG_NOSIGNAL);
    free(toWriteC);

    if (sent < 0) {
        if (conn->isNew) {
            last_error = "send() failed: " + string(strerror(errno));
            destroyConn(conn);
            request->conn = NULL;
            finalizeError(request);
        } else {
...
void EvHttpClient::readCb(struct ev_loop *loop, struct ev_io *watcher, int revents) {
    if (EV_ERROR & revents) {
        last_error = "got invalid read event: " + string(strerror(errno));
        return;
    }
    ...
    if (received <= 0) {
        if (conn->isNew) {
            last_error = "recv() failed: " + string(strerror(errno));
            destroyConn(conn);
            request->conn = NULL;
            finalizeError(request);
        } else {
...
void EvHttpClient::clearLastError() {
    last_error = "";
}

...
void append(BinLogWriter *writer, int count)
{
...
void read(BinLogReader *reader, int count, uint64_t binlog_id = 0)
{
...
void read_last(BinLog *binlog, BinLogReader *reader)
{
    read(reader, 1, binlog->last_binlog_id());
}

void bug1(BinLog *binlog)
{
    // bug: the map pos of MmapReadableFile cannot be updated
